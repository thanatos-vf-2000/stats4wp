...
class Animator {
	...
	_refresh() {
		...
		this._running         = true;
		this._request         = requestAnimFrame.call(
			window,
			() => {
            this._update();
            this._request = null;
...
var Interaction                    = {
	...
	modes: {
		index( chart, e, options, useFinalPosition ) {
			...
			chart.getSortedVisibleDatasetMetas().forEach(
				(meta) => {
...
function placeBoxes(boxes, chartArea, params, stacks) {
	...
	function mergeScaleConfig(config, options) {
		...
		Object.keys( configScales ).forEach(
			(id) => {
		...
		config.data.datasets.forEach(
            ...
            Object.keys( defaultScaleOptions ).forEach(
					(defaultID) => {
	...
	class Chart {
		...
		buildOrUpdateScales() {
				...
				const updated   = Object.keys( scales ).reduce(
					(obj, id) => {
                    obj[id] = false;
                    return obj;
					},
		...
		_getUniformDataChanges() {
			...
			return Array.from( changeSet ).map( (c) => c.split( ',' ) ).map(
				(a) => ({
                method: a[1],
                start: +a[2],
                count: +a[3]
...
class ArcElement extends Element {
	...
	inRange(chartX, chartY, useFinalPosition) {
		...
		const { angle , distance  } = getAngleFromPoint(
			point,
			{
				x: chartX,
				y: chartY
	...
	defaults: {
		...
		labels: {
			...
			generateLabels( chart ) {
				...
				return chart._getSortedDatasetMetas().map(
					(meta) => {
...
class LogarithmicScale extends Scale {
	...
	parse(raw, index) {
		const value = LinearScaleBase.prototype.parse.apply(
			this,
			[
			raw,
			index
...
class RadialLinearScale extends LinearScaleBase {
	...
	generateTickLabels(ticks) {
		...
		this._pointLabels   = this.getLabels().map(
			(value, index) => {
