...
$last_hour = $wpdb->get_row("SELECT count(*) as visitors, COALESCE(SUM(hits),0) as pages
    FROM ". DB::table('visitor') ." 
    where device!='bot' and location != 'local'
...
<div class="rows stats4wp-dashboard-header">
    ...
    <div class="stats4wp-db-container">
        <div class="stats4wp-db-box fade">
            <div class="stats4wp-db-label"><?php _e('Total visitors', 'stats4wp'); ?></div>
        ...
        <div class="stats4wp-db-box fade">
            <div class="stats4wp-db-label"><?php _e('Total pageviews', 'stats4wp'); ?></div>
        ...
        <div class="stats4wp-db-box fade">
            <div class="stats4wp-db-label"><?php _e('Realtime visitors', 'stats4wp'); ?></div>
            <div class="stats4wp-db-amount"><?php echo esc_html(number_format($last_hour->visitors, 0, ',', ' ')); ?></div>
            <div class="stats4wp-db-compare"><span><span><?php _e('visitors in the last hour');?></span></span></div>
        </div>
        <div class="stats4wp-db-box fade">
            <div class="stats4wp-db-label"><?php _e('Realtime pageviews', 'stats4wp'); ?></div>
            <div class="stats4wp-db-amount"><?php echo esc_html(number_format($last_hour->pages, 0, ',', ' ')); ?></div>
            <div class="stats4wp-db-compare"><span><span><?php _e('pageviews in the last hour');?></span></span></div>
        </div>
    </div>
    <?php
